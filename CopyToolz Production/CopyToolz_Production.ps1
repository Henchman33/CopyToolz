...
# FIX 2 — highlight colors for selected radio button
$clrRadioSel   = [System.Drawing.Color]::FromArgb(0, 188, 140)      # bright teal bg when selected
$clrRadioSelFg = [System.Drawing.Color]::FromArgb(10, 10, 14)       # near-black text on teal
...
function New-FlatButton {
    ...
    $btn.FlatAppearance.MouseOverBackColor = [System.Drawing.Color]::FromArgb(
        [math]::Min(255, $Bg.R + 25), [math]::Min(255, $Bg.G + 25), [math]::Min(255, $Bg.B + 25))
...
# ── OPERATION MODE ───────────────────────────────────────────
# FIX 2: Radio buttons now highlight with a solid teal background when selected.
# Each radio button's CheckedChanged handler updates all three siblings.

$lblMode = New-SectionLabel "OPERATION MODE" 16 158
$card.Controls.Add($lblMode)

# Helper to update all three radio button appearances
function Update-RadioStyles {
    foreach ($rb in @($rbCopy, $rbBackup, $rbMove)) {
        if ($rb.Checked) {
            $rb.BackColor = $clrRadioSel
            $rb.ForeColor = $clrRadioSelFg
            $rb.Font      = $fontSmallBold
        } else {
...
$rbCopy.Location = New-Object System.Drawing.Point(16, 178)
$rbCopy.Size     = New-Object System.Drawing.Size(190, 24)
$rbCopy.Checked  = $true
$rbCopy.Font     = $fontSmallBold
$rbCopy.ForeColor = $clrRadioSelFg
$rbCopy.BackColor = $clrRadioSel
...
$rbBackup.Location = New-Object System.Drawing.Point(214, 178)
$rbBackup.Size     = New-Object System.Drawing.Size(240, 24)
...
$rbMove.Location = New-Object System.Drawing.Point(462, 178)
$rbMove.Size     = New-Object System.Drawing.Size(240, 24)
$rbMove.Font     = $fontSmall
$rbMove.ForeColor = $clrText
...
# Wire up CheckedChanged after all three exist
$rbCopy.Add_CheckedChanged({   if ($rbCopy.Checked)   { Update-RadioStyles } })
$rbBackup.Add_CheckedChanged({ if ($rbBackup.Checked) { Update-RadioStyles } })
$rbMove.Add_CheckedChanged({   if ($rbMove.Checked)   { Update-RadioStyles } })
...
$btnStart.Add_Click({
    ...
    if (-not $source)      { [System.Windows.Forms.MessageBox]::Show("Please enter a source path.",      "CopyToolz", "OK", "Warning"); return }
    if (-not $destination) { [System.Windows.Forms.MessageBox]::Show("Please enter a destination path.", "CopyToolz", "OK", "Warning"); return }
    ...
    # FIX 1 — Include the source root folder name in the destination path.
    # Previously, files in the root of $source were copied directly into $destination
    # with no subfolder, so the root folder itself was never recreated.
    $sourceFolderName = Split-Path $source.TrimEnd('\') -Leaf
    $destinationRoot  = Join-Path $destination $sourceFolderName

    # ── Backup mode: append timestamp subfolder ───────────────
    if ($rbBackup.Checked) {
        $destinationRoot = Join-Path $destination ("Backup_" + (Get-Date -Format "yyyyMMdd_HHmmss") + "_" + $sourceFolderName)
        Write-ColorLog "Backup destination: $destinationRoot" "INFO"
    }
    ...
    if ($total -eq 0) { Write-ColorLog "No files matched filter '$filter' in source." "WARN" }
